...
For readable two-panel figures, require `--plot-alpha-range MIN:MAX` and `--plot-gamma-range MIN:MAX` for `analyze` and `all`; there are no default display ranges. Select the inclusive proposed-method subset only from the alpha/gamma values already listed in the YAML experiment grid, state the selected endpoints in the title, and always retain the complete baseline alpha curve/reference points in both panels. This is a visual filter only: the complete alpha/gamma grid remains in all CSV outputs, Pareto/frontier computation, matched-diversity inference, and the standalone primary baseline figure.
